...
public class SmartWordToyTest{
  ...
  private final String TEST_NAME = "SRM233_DIV1";
  private final String PROBLEM_NAME = "250";
  private final String METHOD_NAME = "minPresses";
  ...
  private void doTest(int testNo) throws IOException {
    ...
    String end = sc.nextLine().replace("\"", "");
    String forbid = sc.nextLine();
    int answerVal = 0;
    String[] stringArray = forbid.replace("\"", "").replace("{","").replace("}","").split(",");

    try {
      SmartWordToy target = new SmartWordToy();
      ...
      for(Method m: methods){
        if(Modifier.PUBLIC == m.getModifiers() && m.getName().equals(METHOD_NAME)){
          ...
          answerVal = ((Integer) m.invoke(target, start, end,stringArray)).intValue();
